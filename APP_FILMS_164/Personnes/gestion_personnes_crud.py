        ...
        try:
            with DBconnection() as mc_afficher:
                if order_by == "ASC" and id_genre_sel == 0:
                    ...
                    strsql_genres_afficher = """SELECT * FROM t_vehicule"""
...
def genre_update_wtf():
    ...
    try:
        ...
        if request.method == "POST" and form_update.submit.data:
            ...
            str_sql_update_intitulegenre = """UPDATE t_vehicule SET modele_vehicule = %(value_name_genre)s, 
            date_ins_genre = %(value_date_genre_essai)s WHERE id_vehicule = %(value_id_genre)s """
            with DBconnection() as mconn_bd:
            ...
            str_sql_id_genre = "SELECT id_vehicule, modele_vehicule, imatriculation FROM t_vehicule " \
                               "WHERE id_vehicule = %(value_id_genre)s"
            valeur_select_dictionnaire = {"value_id_vehicule": id_genre_update}
            with DBconnection() as mybd_conn:
                mybd_conn.execute(str_sql_id_genre, valeur_select_dictionnaire)
            # Une seule valeur est suffisante "fetchone()", vu qu'il n'y a qu'un seul champ "nom genre" pour l'UPDATE
            data_nom_genre = mybd_conn.fetchone()
            print("data_nom_genre ", data_nom_genre, " type ", type(data_nom_genre), " genre ",
                  data_nom_genre["modele_vehicule"])

            # Afficher la valeur sélectionnée dans les champs du formulaire "genre_update_wtf.html"
            form_update.nom_genre_update_wtf.data = data_nom_genre["modele_vehicule"]
            form_update.date_genre_wtf_essai.data = data_nom_genre["imatriculation"]
    ...
    try:
        ...
        if request.method == "POST" and form_delete.validate_on_submit():
            ...
            if form_delete.submit_btn_del.data:
                ...
                str_sql_delete_louer = """DELETE FROM t_louer WHERE fk_vehicule = %(value_id_genre)s"""
                str_sql_delete_retourner = """DELETE FROM t_retourner WHERE fk_vehicule = %(value_id_genre)s"""
                str_sql_delete_vehicule_periode = """DELETE FROM t_vehicule_periode WHERE fk_vehicule = %(value_id_genre)s"""
                str_sql_delete_vehicule_garage = """DELETE FROM t_vehicule_garage WHERE fk_vehicule = %(value_id_genre)s"""
                str_sql_delete_idgenre = """DELETE FROM morattel_bryan_expi1a_mpd.t_vehicule WHERE id_vehicule = %(value_id_genre)s"""
                ...
                with DBconnection() as mconn_bd:
                    ...
                    mconn_bd.execute(str_sql_delete_vehicule_garage, valeur_delete_dictionnaire)
        ...
        if request.method == "GET":
            ...
            with DBconnection() as mydb_conn:
                ...
                # Opération sur la BD pour récupérer "id_genre" et "intitule_genre" de la "t_genre"
                str_sql_id_genre = "SELECT id_vehicule, modele_vehicule FROM t_vehicule WHERE id_vehicule = %(value_id_genre)s"
