    ...
    if request.method == "POST":
        try:
            if form_add_film.validate_on_submit():
                ...
                strsql_insert_film = """INSERT INTO t_garage (id_garage,adresse_garage) VALUES (NULL,%(value_nom_film)s) """
...
def film_update_wtf():
    ...
    try:
        ...
        if request.method == "POST" and form_update_film.submit.data:
            ...
            str_sql_update_nom_film = """UPDATE t_garage SET adresse_garage = %(value_nom_film)s,
                                                            nbr_place_total = %(value_duree_film)s
                                                            WHERE id_garage = %(value_id_film)s"""
            with DBconnection() as mconn_bd:
            ...
            str_sql_id_film = "SELECT * FROM t_garage WHERE id_garage = %(value_id_film)s"
            ...
            print("data_film ", data_film, " type ", type(data_film), " genre ",
                  data_film["adresse_garage"])

            # Afficher la valeur sélectionnée dans le champ du formulaire "film_update_wtf.html"
            form_update_film.nom_film_update_wtf.data = data_film["adresse_garage"]
            form_update_film.duree_film_update_wtf.data = data_film["nbr_place_total"]
            # Debug simple pour contrôler la valeur dans la console "run" de PyCharm
            print(f" duree film  ", data_film["nbr_place_total"], "  type ", type(data_film["nbr_place_total"]))
...
def film_delete_wtf():
    ...
    try:
        ...
        if form_delete_film.submit_btn_del_film.data:
            ...
            str_sql_delete_fk_film_genre = """DELETE FROM t_vehicule_garage WHERE fk_garage = %(value_id_film)s"""
            str_sql_delete_film = """DELETE FROM t_garage WHERE id_garage = %(value_id_film)s"""
            ...
            flash(f"Garage définitivement effacé !!", "success")
            print(f"Garage définitivement effacé !!")
        ...
        if request.method == "GET":
            ...
            str_sql_genres_films_delete = """SELECT * FROM t_garage WHERE id_garage = %(value_id_film)s"""
