    ...
    if request.method == "GET":
        try:
            with DBconnection() as mc_afficher:
                strsql_genres_films_afficher_data = """SELECT id_garage, adresse_garage, nbr_place_total,
                                                            GROUP_CONCAT(modele_vehicule) as ModeleVehicule FROM t_vehicule_garage
                ...
                if id_film_sel == 0:
                    ...
                    strsql_genres_films_afficher_data += """ HAVING id_garage= %(value_id_film_selected)s"""
                ...
                if not data_genres_films_afficher and id_film_sel == 0:
                    ...
                    flash(f"Garages affichés !!!", "success")
...
def edit_genre_film_selected():
    if request.method == "GET":
        try:
            ...
            lst_data_film_selected = [item['id_garage'] for item in data_genre_film_selected]
            ...
            lst_data_genres_films_non_attribues = [item['id_vehicule'] for item in data_genres_films_non_attribues]
            ...
            lst_data_genres_films_old_attribues = [item['id_vehicule'] for item in data_genres_films_attribues]
            ...
            lst_data_genres_films_non_attribues = [item['modele_vehicule'] for item in data_genres_films_non_attribues]
...
def update_genre_film_selected():
    if request.method == "POST":
        try:
            ...
            strsql_insert_genre_film = """INSERT INTO t_vehicule_garage (id_vehicule_garage, fk_vehicule, fk_garage)
            ...
            strsql_delete_genre_film = """DELETE FROM t_vehicule_garage WHERE fk_vehicule = %(value_fk_genre)s AND fk_garage = %(value_fk_film)s"""
...
def genres_films_afficher_data(valeur_id_film_selected_dict):
    ...
    try:
        ...
        strsql_genres_films_attribues = """SELECT id_garage, id_vehicule, modele_vehicule FROM t_vehicule_garage
                                            INNER JOIN t_garage ON t_garage.id_garage = t_vehicule_garage.fk_garage
                                            INNER JOIN t_vehicule ON t_vehicule.id_vehicule = t_vehicule_garage.fk_vehicule
                                            WHERE id_garage = %(value_id_film_selected)s"""
